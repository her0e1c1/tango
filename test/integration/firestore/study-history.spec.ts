...
describe("HISTORY-01 HISTORY-04 Firestore history reads", () => {
  afterAll(async () => {
  ...
  it("reads the entire period with both optional Deck constraints and delivers offline cache snapshots", async () => {
    ...
    expect((await read(deckId, "started")).records).toHaveLength(130);
    expect((await read(deckId, "completed")).records).toEqual([
      { deckId, startedAt: start - 1, endedAt: start, endReason: "completed" },
    ]);

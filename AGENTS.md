...
- Entity `model/store.ts` owns Entity state and synchronous updates, including reads within updates. Put store-backed read operations in `model/queries.ts`. Keep Entity React hooks in `model/hooks.ts`. Keep schemas and rules pure, and persistence implementations in `api/`.

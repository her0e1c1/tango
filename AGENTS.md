...
- In Pages and Features, put state-changing operations and workflows in `model/actions/`, and read-only getters, selectors, and derived data in `model/queries/`. Queries must not update state or initiate persistence.
- Give each Page or Feature action its own file and ordinary named function. Keep implementations out of actions objects, action factories, and hooks.
- In Entities, keep synchronous store getters, selector hooks, and state updates in `model/store.ts`; do not create Entity `model/actions/` or `model/queries/`.
...
- Keep Entity state, initialization, synchronous state access, and persistence middleware in `model/store.ts`. Keep schemas and rules pure, and persistence implementations in `api/`.

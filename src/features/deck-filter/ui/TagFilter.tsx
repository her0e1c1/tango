...
export const TagFilter: React.FC<TagFilterProps> = (props) => {
  ...
  return (
    ...
    >
      ...
      <TagMatchOptions matchAll={props.matchAll} onMatchAllChange={props.onMatchAllChange} matchAnyRef={matchAnyRef} />
      ...
      <TagChoices
        ...
        expanded={expanded}
        onToggleTag={(tag) => toggleFilterTag(tag, props, expanded, tagRefs.current, matchAnyRef.current)}
...
function toggleFilterTag(
  tag: string,
  props: Pick<TagFilterProps, "tags" | "selectedTags" | "onSelectedTagsChange">,
  expanded: boolean,
  tagRefs: Map<string, HTMLInputElement>,
  matchAny: HTMLInputElement | null
) {
  const selectedTags = uniqueInOrder(props.selectedTags);
  const selectedTagSet = new Set(selectedTags);
  ...
  const nextUnselectedTags = uniqueInOrder(props.tags).filter((availableTag) => !nextSelectedTagSet.has(availableTag));
  ...
  if (!nextVisibleTags.includes(tag)) {
    ...
    const nextFocusableTag = nextVisibleTags.find((visibleTag) => tagRefs.has(visibleTag));
    (nextFocusableTag === undefined ? matchAny : tagRefs.get(nextFocusableTag))?.focus();
  }

  props.onSelectedTagsChange(nextSelectedTags);
}

function TagMatchOptions({
  matchAnyRef,
  ...props
}: Pick<TagFilterProps, "matchAll" | "onMatchAllChange"> & { matchAnyRef: React.RefObject<HTMLInputElement | null> }) {
...
function TagFilterHeader({
  ...
  const status =
    selectedCount === 0
      ...
      : t("deckFilter.tagFilter.selected", { count: selectedCount });
...
interface TagChoicesProps {
  ...
  tagRefs: React.RefObject<Map<string, HTMLInputElement>>;

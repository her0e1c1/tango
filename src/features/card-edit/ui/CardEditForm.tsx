...
export interface CardEditFormProps {
  card: Card;
  editCard?: ((uid: string, card: CardEdit) => Promise<void>) | undefined;
  onCancel: () => void;
  onSaved: () => void;
}

export const CardEditForm: React.FC<CardEditFormProps> = ({ card, editCard, onCancel, onSaved }) => {
  const editAction = useCardEditAction({ editCard, onSaved });

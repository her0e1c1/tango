...
- Name the route entry and composition boundary `*Page.tsx`. Keep route params and screen shortcuts in Pages or Containers, and obtain application state and bound action callbacks from the Page model.
- Keep page-specific imperative navigation and its operation-specific guards in ordinary functions under `model/actions/`, using the context-free navigation API and guard primitives from Shared. Page model hooks only bind action inputs; keep `Link` / `NavLink` and guard UI rendering in `ui/`.

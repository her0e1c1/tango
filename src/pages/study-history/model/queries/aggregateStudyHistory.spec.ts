...
const record = (
  ...
  deckId = "deck"
): StudyHistoryRecord => ({ deckId, startedAt, endedAt, endReason });
const visible = new Set(["deck"]);

describe("HISTORY-01 daily study counts", () => {
  it("counts a completed session once in both metrics and includes 30 calendar days", () => {
    const session = record(new Date(2026, 8, 21, 10).getTime(), new Date(2026, 8, 21, 11).getTime(), "completed");
    const result = aggregateStudyHistory(period, [session], [session], visible);
  ...
  it("counts starts and completions independently across midnight and the inclusive/exclusive bounds", () => {
    const crossDay = record(new Date(2026, 8, 20, 23).getTime(), new Date(2026, 8, 21, 1).getTime(), "completed");
    const sessions = [
      crossDay,
      record(period.start - 1, period.start, "completed"),
      record(period.start),
      record(period.end - 1),
      record(period.end, period.end, "completed"),
      record(period.start, period.start + 1, "abandoned"),
    ];
    const result = aggregateStudyHistory(period, sessions, sessions, visible);
  ...
  it("retains more than 100 records and excludes unavailable decks without displacing visible records", () => {
    ...
    const hidden = Array.from({ length: 200 }, () => record(period.start, period.start, "completed", "deleted"));

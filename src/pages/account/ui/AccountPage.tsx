...
export const AccountPage: React.FC = () => {
  ...
  const isLoggedIn = currentUser != null && !currentUser.isAnonymous;
          ...
          <dl className="divide-y divide-border">
            ...
            <div className="flex min-h-touch items-start justify-between gap-4 px-4 py-3">
              ...
              <dd className="min-w-0 break-words text-right text-caption text-ink-muted">
                {isLoggedIn ? (currentUser.displayName ?? "No name") : "Not available"}
              </dd>
            </div>
            <div className="flex min-h-touch items-start justify-between gap-4 px-4 py-3">
              <dt className="shrink-0 text-body font-medium text-ink">User ID</dt>
              <dd className="min-w-0 break-all text-right text-caption text-ink-muted">{currentUser?.uid ?? ""}</dd>
            </div>
          </dl>

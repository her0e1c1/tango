...
import { omitUndefined, parseDocument, timestampOrDateSchema } from "@/shared/firebase/document-validation";
...
const cardCreateDtoSchema = cardDocumentSchema.extend({ id: z.string() });
const cardUpdateDtoSchema = cardDocumentSchema.omit({ id: true }).partial().extend({ updatedAt: z.number() });

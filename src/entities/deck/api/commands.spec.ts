...
describe("deck commands", () => {
  ...
  it("rejects missing users and mismatched owners before writing", async () => {
    ...
    await expect(deckCommands.update("uid-a", createDeck({ uid: "uid-b" }))).rejects.toThrow("owner does not match");
  ...
  it("serializes writes to the same Deck", async () => {
    ...
    const secondUpdate = deckCommands.update("uid-a", { ...deck, name: "second" });

...
| Firebase 初期化 | `src/firebase.ts`, `.env.example`, `.env.e2e`, `.devcontainer/compose.yaml`, `.devcontainer/compose.e2e.yaml` | Firebase app 初期化、開発時 Firestore emulator 接続、e2e 用設定 |
| sample 生成 | `sample/*` | Python テストファイルから初期カード JSON を生成し、`sample/build/output.json` をアプリ初期データに使う |
| 実行・CI | `package.json`, `Makefile`, `.devcontainer/compose.yaml`, `.github/workflows/*` | dev server、build、lint、test、Firebase Hosting deploy、Docker image 更新 |

...
| `npm run lint:react` | `package.json`, `eslint.config.mjs` | React Hooks、React Compiler、feature 間依存の診断（warning も失敗） |
...
Biome は formatter と一般 lint の source of truth のまま維持します。ESLint は `src/**/*.{ts,tsx}` に限定し、`eslint-plugin-react-hooks` の `recommended-latest` と `eslint-plugin-boundaries` を実行します。これにより Rules of Hooks に加え、Compiler の `purity`、`immutability`、`refs`、`set-state-in-effect`、`incompatible-library`、feature 間の直接 import を `npm run lint:react` で検証します。feature 内の import、共有モジュールへの import、feature 外から feature への import は許可します。`npm run lint` と `make check` からも実行し、warning を許可しません。

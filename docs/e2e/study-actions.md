...
- 評価回答では StudyAnswer の作成、Card.fsrs と updatedAt の部分更新、StudySession の前進・完了を一つの batch にまとめる。ID と回答時刻は受付時に固定する。
...
- 回答形式、参照先、回答 ID、Card の FSRS、回答順序、Session の前進・完了はアプリとそのテストの責務とする。Rules は本人 UID の回答作成を許可し、payload や参照先の存在・整合、Session の状態遷移は制約しない。
...
- FSRS は Card.fsrs に統合する。未評価は null とし、作成・複製では null に初期化し、本文編集・既存 Card への更新 import では保持する。公開 Card の読取権限は FSRS にも適用する。fsrs は状態、期限・前回評価時刻（Unixミリ秒）、stability、difficulty、reps、lapses、scheduledDays、learningSteps を保持し、reload 後の次回計算を変えない。fsrs 内の version や elapsedDays は保存しない。
...
- `again` / `hard` / `good` / `easy` は受け付けた値のまま保存し、FSRS の計算に使用する。Card の内容と createdAt は維持し、updatedAt は評価時刻になる。
- good / hard / easy の各評価から FSRS を生成する。Card.fsrs が null でなければその記憶状態を更新し、null なら空の初期状態から計算する。旧 Card の studySchedule や閲覧履歴は移行・参照しない。
- Card の内容と createdAt は変わらず、Card.fsrs.reps が1増え、updatedAt は今回の評価時刻となる。
- 回答・Card・session の前進はすべて保存されるか、いずれも保存されない。
...
- アプリは 操作受付時に固定した独立したランダム回答 ID を使い、回答、Card、session の更新を一つの batch にまとめ、通常の二重送信を防止する。
...
- 回答Documentを作成しない。
- FSRS scheduleを生成・変更しない。裏面表示、Help、離脱、slider、autoplay でも Card.fsrs を更新しない。
- Card.fsrs を更新せず、Card の内容・メタデータも変更しない。Session だけを前進する。
- session の位置が次の Card へ進む。
- 次の Card の front text が表示される。
- browser error が発生しない。

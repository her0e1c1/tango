...
通常ログインの準備は Emulator REST API、保存確認は対象 UID / ID / 更新値の polling を使う。
匿名ケースは UI で作成・インポート・学習し、表示・CSV ダウンロード・同じブラウザの reload で永続化を確認する。
サーバー不在だけを成功条件にせず、実際のデータ表示と併せて検証する。
ブラウザ内の Firebase インスタンスをテストから操作する専用 bundle は使用しない。

...
- 対応する文字コードは UTF-8 と形式説明に表示する。不正な UTF-8 の元バイト列は解析前に拒否し、英語・日本語で UTF-8 での再保存と再選択を案内する。preview・保存準備・Deck・Card は作成せず、以前の選択にも戻らない。処理中の排他を解除し、有効なファイルの再選択で回復できる。

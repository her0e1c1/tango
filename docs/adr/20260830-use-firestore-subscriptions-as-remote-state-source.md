...
各listener callbackでは、受け取ったsnapshot全体をatomicにparseし、accepted snapshotでRemote collection全体をreplaceする。`documentChanges()`によるincremental mirrorやmutation metadataの別Storeを維持しない。invalid documentを含むsnapshotはpartial publishしない。

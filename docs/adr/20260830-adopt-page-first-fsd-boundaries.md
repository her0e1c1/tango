...
Page UIでPage modelが提供する値とcallbackをpresentational componentへ接続できる境界は`*Page`と`*Container`に限定し、それ以外のUIコンポーネントはprops-drivenとする。

Page model hookをlower-layer stateとworkflowのcompositionおよびwiring boundaryとする。更新と非同期workflowは個別のaction、read-onlyな参照はquery、React state、form、ref、cleanupはstate hookが所有する。Page model hookがこれらを接続して値とbound callbackを`*Page`または`*Container`へ提供する。

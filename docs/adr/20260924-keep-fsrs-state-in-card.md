...
- 通常アカウントの操作完了は batch のサーバー保存成功を基準とし、失敗は呼び出し側が toast で通知する。匿名利用ではサーバー応答を待たない。オフライン中の保留とローカル反映は[エラー伝播の方針](./20260924-propagate-firestore-write-errors.md)に従う。

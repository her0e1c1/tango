...
- 保存中の二重操作を防ぎ、ユーザー変更後の古い通知と、画面変更後の古い遷移・表示状態更新を抑止する。画面を離れた後の全体向け通知は既存の操作寿命の契約に従う。操作のロックは書き込み Promise の成功・失敗で解除する。学習画面は rollback 完了を追跡せず、表示する Card と進捗を購読結果から取得する。匿名書き込みの遅延エラーは通知だけを行い、後続の操作ロックを変更しない。

- Deck 削除は Deck の保存結果で成否を確定する。学習セッションの終了は独立した後処理とし、終了を待って削除操作をロックしない。後処理の失敗は削除失敗として扱わず、専用の toast で通知する。

...
- 匿名利用では Firestore 通信を停止したまま、サーバー応答を待たずに操作を返す。公開 API に書き込みエラー用 callback は設けない。待機方針だけを低レベル helper に集約し、匿名書き込みの遅延エラーはブラウザー標準の `reportError` へ渡し、無言で破棄しない。入力検証などの失敗は引き続き Promise の reject で伝える。書き込み API の返却と表示への反映は区別し、表示はローカル snapshot に従う。

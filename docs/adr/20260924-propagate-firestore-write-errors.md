...
- 匿名利用では Firestore 通信を停止したまま、サーバー応答を待たずに操作を返す。呼び出し側は `onLocalError` callback を渡し、後から検出された失敗を toast で通知する。入力検証などの失敗は引き続き Promise の reject で伝える。書き込み API の返却と表示への反映は区別し、表示はローカル snapshot に従う。

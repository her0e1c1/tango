...
進行・完了は評価を伴わない本番の学習保存操作で確認し、本人の Deck と Card をアプリケーションでも参照できる状態にする。
...
| ID | カテゴリ | 区分 | テストケース |
| --- | --- | --- | --- |
| FIRESTORE-STUDY-SESSION-01 | batch | 正常系 | [途中の学習を保存した順序と位置から再開できる](#firestore-study-session-01) |
...
| FIRESTORE-STUDY-SESSION-09 | read | 正常系 | [古い学習の更新時刻に惑わされずDeckごとに最新のセッションを復元する](#firestore-study-session-09) |
| FIRESTORE-STUDY-SESSION-10 | read | 正常系 | [最新の学習が終了済みなら古い未終了セッションも復元しない](#firestore-study-session-10) |
| FIRESTORE-STUDY-SESSION-11 | read | 正常系 | [購読をやり直さず保存された進行位置を反映する](#firestore-study-session-11) |
...
- 重複した操作はセッション不一致のエラーで reject し、保存内容は最初の進行保存後と同一である。3枚目へは進まない。
...
- 再開する時刻は、直前に保存した lastStudiedAt より後である。
...
- lastStudiedAt が再開時刻、updatedAt がサーバー確定時刻に変わり、session ID・カード順序・位置・開始日時・作成日時・終了状態は変わらない。
...
| セッション | Deck | 開始日時・作成日時 | 最終学習日時 | 位置 | 終了状態 |
| --- | --- | --- | --- | --- | --- |
...
- 対象 Deck は新セッションの ID と位置 `2` を再開対象にする。旧セッションの最終学習日時の方が新しくても選び直さない。
...
- 終了した新セッションの位置 `2` に対する遅延した進行更新を、本番の進行書き込み操作で batch に追加して保存し、その後、再購読する。

Then:

- 最後のカードの完了操作は成功を返し、中断操作も正常に完了する。遅延更新後も新セッションの終了理由と終了日時は維持される。
...
- アプリケーションの進行操作を経由せず、SDK から保存済みセッションの位置を `1` に変更し、lastStudiedAt と serverTimestamp の updatedAt を設定する。

Then:

- 購読を解除・再登録せずに、再開対象の位置が `1` になる。
- session ID とカード順序は変わらず、最終学習時刻は保存した lastStudiedAt になる。
...
- 完了操作の Promise はオフライン中に完了せず、再接続後の保存成功で完了したセッションと終了理由 `completed` に resolve する。
...
- 元の UID での再開始、位置 `1` への変更、次のカードへの進行保存、中断、再開時刻の更新をそれぞれ要求する。
...
- StudySession の購読を明示的に開始してから、学習日時を過去の値として保存し、サーバーの保存成功を待ってからセッションを進めて完了する。一方の履歴購読だけを停止し、別 UID の履歴購読を追加する。

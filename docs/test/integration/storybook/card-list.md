...
| STORYBOOK-CARD-LIST-02 | interaction | [難易度の一括変更を要求する](#storybook-card-list-02) | CardList :: `BulkDifficulty` |
| STORYBOOK-CARD-LIST-03 | render | [Card 未作成の空状態を表示する](#storybook-card-list-03) | CardList :: `Empty` |
| STORYBOOK-CARD-LIST-04 | render | [フィルターによる0件状態に解除導線を表示する](#storybook-card-list-04) | CardList :: `FilterZero` |
| STORYBOOK-CARD-LIST-05 | render | [復習期限による0件状態をフィルター不一致と区別する](#storybook-card-list-05) | CardList :: `IntervalZero` |
| STORYBOOK-CARD-LIST-06 | interaction | [選択した Card の ID を閲覧 callback に渡す](#storybook-card-list-06) | CardList :: `ViewCard` |
| STORYBOOK-CARD-LIST-07 | interaction | [選択済みタグを解除して表示を更新する](#storybook-card-list-07) | CardList :: `RemovableSelectedTags` |
| STORYBOOK-CARD-LIST-08 | interaction | [Card の overlay を閉じる](#storybook-card-list-08) | CardList :: `CardViewInteraction` |
| STORYBOOK-CARD-LIST-09 | interaction | [標準順への変更を要求する](#storybook-card-list-09) | CardList :: `NewestAdded` |
| STORYBOOK-CARD-LIST-10 | interaction | [Card の編集を要求して操作メニューを閉じる](#storybook-card-list-10) | CardActionsMenu :: `Interaction` |
| STORYBOOK-CARD-LIST-11 | render | [空理由の指定がなければ空状態の案内を断定しない](#storybook-card-list-11) | CardList :: `UnspecifiedEmptyReason`（未実装） |
| STORYBOOK-CARD-LIST-12 | render | [長い選択タグの文字列を変更しない](#storybook-card-list-12) | CardList :: `LongSelectedTag`（未実装） |
| STORYBOOK-CARD-LIST-13 | interaction | [タグ解除後に残ったタグへフォーカスを移す](#storybook-card-list-13) | CardList :: `RemoveTagKeyboard`（未実装） |
| STORYBOOK-CARD-LIST-14 | interaction | [最後のタグ解除後はフィルターの見出しへ戻る](#storybook-card-list-14) | CardList :: `RemoveLastTagKeyboard`（未実装） |
| STORYBOOK-CARD-LIST-15 | interaction | [Tab 移動でタグ選択を変更しない](#storybook-card-list-15) | CardList :: `TagTabOrder`（未実装） |
| STORYBOOK-CARD-LIST-16 | interaction | [メニューを一つに保ち削除された行のメニューを閉じる](#storybook-card-list-16) | CardList :: `MenuLifecycle`（未実装） |
| STORYBOOK-CARD-LIST-17 | interaction | [行の並べ替え後も同じ Card を操作する](#storybook-card-list-17) | CardList :: `ReorderFocus`（未実装） |
| STORYBOOK-CARD-LIST-18 | interaction | [空状態の追加ボタンから作成を要求する](#storybook-card-list-18) | CardList :: `EmptyAddAction`（未実装） |
| STORYBOOK-CARD-LIST-19 | interaction | [0件状態からフィルター解除を要求する](#storybook-card-list-19) | CardList :: `EmptyClearAction`（未実装） |
| STORYBOOK-CARD-LIST-20 | interaction | [Card 行の編集要求に対象 ID を渡す](#storybook-card-list-20) | Card :: `EditTarget`（未実装） |
| STORYBOOK-CARD-LIST-21 | render | [保存中の Card 行の閲覧と操作を無効にする](#storybook-card-list-21) | Card :: `PendingContract`（未実装） |
| STORYBOOK-CARD-LIST-22 | interaction | [Card の削除を要求する](#storybook-card-list-22) | CardActionsMenu :: `DeleteAction`（未実装） |
| STORYBOOK-CARD-LIST-23 | render | [無効な操作メニューを表示しない](#storybook-card-list-23) | CardActionsMenu :: `DisabledContract`（未実装） |

対応ファイルは [CardList.stories.tsx](../../../../src/pages/card-list/ui/CardList.stories.tsx)、[CardActionsMenu.stories.tsx](../../../../src/pages/card-list/ui/CardActionsMenu.stories.tsx)。Card は `src/pages/card-list/ui/Card.stories.tsx` を対応予定ファイルとする。11 以降の named export は追加予定であり、既存 `play` の検証済み項目ではない。
...
対応 Story: CardList :: `BulkDifficulty`
...
### STORYBOOK-CARD-LIST-11 空理由の指定がなければ空状態の案内を断定しない
...
### STORYBOOK-CARD-LIST-12 長い選択タグの文字列を変更しない
...
### STORYBOOK-CARD-LIST-13 タグ解除後に残ったタグへフォーカスを移す
...
### STORYBOOK-CARD-LIST-14 最後のタグ解除後はフィルターの見出しへ戻る
...
### STORYBOOK-CARD-LIST-15 Tab 移動でタグ選択を変更しない
...
### STORYBOOK-CARD-LIST-16 メニューを一つに保ち削除された行のメニューを閉じる
...
### STORYBOOK-CARD-LIST-17 行の並べ替え後も同じ Card を操作する
...
### STORYBOOK-CARD-LIST-18 空状態の追加ボタンから作成を要求する
...
### STORYBOOK-CARD-LIST-19 0件状態からフィルター解除を要求する
...
### STORYBOOK-CARD-LIST-20 Card 行の編集要求に対象 ID を渡す
...
### STORYBOOK-CARD-LIST-21 保存中の Card 行の閲覧と操作を無効にする
...
### STORYBOOK-CARD-LIST-22 Card の削除を要求する
...
<a id="storybook-card-list-23"></a>

### STORYBOOK-CARD-LIST-23 無効な操作メニューを表示しない
...
`FilterSaving`、`CardSaving` と各メニューの `Disabled` は表示専用である。追加予定の21・23を書いたことだけでは、これらの Story の操作抑止を検証済みにはしない。

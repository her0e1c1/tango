...
- オフラインでタグを変更して Card を保存し、編集画面を開き直す。編集画面を再読込してから再接続する。

Then:

- オフライン中は保存中のまま編集画面に留まり、成功後の遷移を行わない。再読込後もローカルの編集内容を表示し、再接続後に同じ内容を同期する。他の Card のタグは変更されない。
...
- オフラインでタグを追加・変更して Card を保存し、一覧の操作メニューから同じ Card の編集画面を開き直して再読込・再接続する。

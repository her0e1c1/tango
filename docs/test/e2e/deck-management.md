...
- 通常アカウントの作成はサーバー保存の成功まで待ち、保存失敗時は入力を維持して toast を表示する。匿名利用ではサーバー応答を待たない。
- クラウドから拒否された Deck は Firestore の rollback により利用できる Deck として一覧に残らない。拒否は呼び出し側が toast で通知する。

...
- 通常アカウントの作成・編集・削除・インポート・回答は、サーバーの保存成功を待って完了する。オフライン中は保存待ちとし、ローカル snapshot への反映と保存成功を区別する。匿名の `local-only` は通信を停止したままサーバー応答を待たずに操作を進め、ローカル snapshot から表示を更新する。
- 保存中にアカウントが変わった場合は、以前の操作の成功・失敗を新しいアカウントへ通知せず、古い結果で画面遷移や現在の選択の終了を行わない。完了した古い処理のロックは解除し、新しい操作を妨げない。
...
- 処理中に画面を離れて再入場しても処理中の表示が維持され、通常アカウントでは Deck とすべての Card のサーバー保存成功後に操作可能になる。匿名利用ではサーバー応答を待たず、ローカル反映を確認する。書き込みの受付だけでは処理完了としない。
- 通常アカウントではサーバーの保存結果を toast で通知する。匿名利用で後から検出された失敗は `reportError` による診断として報告し、操作単位の toast は表示しない。拒否された変更の rollback は Firestore SDK に委ねる。

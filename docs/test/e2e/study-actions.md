...
- 失敗した試行では保存失敗の toast を表示し、swipe feedback は表示しない。成功した再試行だけ swipe feedback を通知する。
- 保存中は評価、スキップ、スライダー、自動再生による移動ができない。回答・位置変更・中止は保存の成功・失敗で操作待ちを解除する。画面は購読結果を表示し、rollback の到着を検出してロックを延長しない。最後の回答で購読上のセッションが消えても、保存中は完了画面へ進まず読み込み表示にする。セッション不在だけでは自動遷移せず、明示的な中止が成功した場合にだけ一覧へ戻る。
- 匿名利用は snapshot の到着やサーバー応答を待たず操作待ちを解除する。後続のローカル保存失敗は `reportError` による診断として報告し、表示は購読結果に従う。遅延エラーでは後続の操作ロックを変更しない。
- オフライン中は保存が保留される。再接続後の保存成功まで追加の評価を受け付けず、再読み込み後も学習結果と現在位置を復元できる。

...
- 保存中は評価、スキップ、スライダー、自動再生による移動ができない。回答・位置変更・中止が拒否された場合は、先行したローカル反映が元の位置へ巻き戻るまで操作と自動遷移を抑止する。
- 匿名利用でローカル保存が失敗し、成功の snapshot が届かない場合も toast を表示して操作待ちを解除し、元の Card から再試行できる。古い失敗通知では後続の操作ロックを解除しない。

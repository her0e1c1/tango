...
- サーバー保存に失敗した場合は失敗の toast を表示し、成功通知や保存後の画面遷移を行わない。準備済みの識別子を保ち、同じ入力で再試行できる。
- クラウドから拒否された Card は Firestore の rollback により利用できる Card として表示されない。保存できた Deck が再読み込みや同期によって重複しない。拒否は呼び出し側が toast で通知する。

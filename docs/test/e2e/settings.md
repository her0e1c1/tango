...
- Dark mode のチェック状態が初期値、反転後、reload 後のそれぞれで設定値と一致する。
...
- 0 の表示値とアクセシブルな値は「条件に一致するすべてのカード」（英語では `All matching cards`）となる。補足説明は 0 が枚数の制限なしを意味し、tags・適用される復習条件による絞り込みは維持されることを説明する。
- 0 でも数値 0 が保存され、reload 後も表示と値が維持される。学習開始画面・start action・新しい session はすべての学習対象 Card を含む。
- 正の上限・0・1 の各設定で、上限を変更して Continue しても既存 session の ID、Card ID と並び順は変わらない。
- browser error が発生しない。
...
- 見出しの名前、開いたときのバージョン・コミット表示が維持される。コミット情報があるビルドではコミットを指すリンク、ないビルドでは既存の `unknown` 表示を維持しリンクは存在しない。
...
- 代表 CSV の有効1件・無効2件・診断3件と元の行の文脈、日本語のユーザー入力を保持し、import は無効のままとする。診断の文脈は言語変更前後で同じ順序・内容となる。

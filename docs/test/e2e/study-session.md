...
- 同じ条件・同じ時点の学習開始画面、Card 一覧、Deck 閲覧で復習対象の判定が一致する。Card 一覧と Deck 閲覧には、学習専用の枚数上限や出題順を適用しない。
- 復習期限の到来、画面への復帰、Card や設定の変更で対象件数が更新される。時計が進んだり戻ったりした場合も、その時点の復習対象が表示される。

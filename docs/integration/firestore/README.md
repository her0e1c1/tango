...
| [Card](./card.md) | 作成、部分更新、StudyProgress、一括保存、論理削除 | `card.spec.ts` |
...
| [Study History](./study-history.md) | 開始・完了履歴の期間・Deck 条件、cache、権限 | `study-history.spec.ts` |

対応テストはすべて [`test/integration/firestore`](../../../test/integration/firestore) に置く。
初版は main の `40e2be11fb80d225fc5444c028741130ca4073ce` にある7ファイルを対象とする。
Issue 起票時の local→remote 移行や local-only session 非送信のケースは、この版の対象テストにはないため検証済みとして記載しない。
既に main にある StudyAnswer の保存・Rules と Study History の取得も対象に含める。
...
| StudyAnswer 保存・認可 | 実際の保存処理と SDK 操作から、回答・Card・session の保存結果と Rules まで | project `test-study-answer`、非匿名認証 UID `answer-owner`。各ケース前にデータと store を初期化し、終了時に Rules 環境を cleanup する |
...
| FIRESTORE-CARD-03 | write | [StudyProgress の更新を Card 本文から分離できる](./card.md#firestore-card-03) |
...
| FIRESTORE-STUDY-ANSWER-03 | write | [保存時の再計算ではなく受理済みの進捗値を使う](./study-answer.md#firestore-study-answer-03) |
...
| FIRESTORE-STUDY-ANSWER-06 | batch | [途中のスキップは回答を作らず進捗と位置を更新する](./study-answer.md#firestore-study-answer-06) |
...
| FIRESTORE-STUDY-ANSWER-14 | write | [回答単独の保存では進捗と session を更新しない](./study-answer.md#firestore-study-answer-14) |
| FIRESTORE-STUDY-ANSWER-15 | write | [境界値の難易度を受理済み進捗として保存できる](./study-answer.md#firestore-study-answer-15) |
...
| ID | カテゴリ | テストケース |
| --- | --- | --- |
| FIRESTORE-SUBSCRIPTIONS-01 | read | [初期 snapshot から Card 本文と学習情報を取得できる](./subscriptions.md#firestore-subscriptions-01) |
...
| FIRESTORE-RULES-05 | batch | [回答作成と Card・session 更新を同じ batch で許可する](./rules.md#firestore-rules-05) |

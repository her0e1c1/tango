...
`lint:test-specs` は各ケースの「対応 Story」にあるファイル・named export と、その Story に直接定義した `play` を静的に確認する。複数ケースでの同じ Story の共有と、一つのケースへの複数 Story の対応を許可する。
対象は同じファイル内の明示的な meta オブジェクトと `export const` の Story オブジェクト、および関数を直接指定した `play` とする。spread・meta 継承、import・別名 export・factory による間接参照は解析せず、対応先として記載されていればエラーにする。
